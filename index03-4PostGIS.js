...
const { Pool, Query } = require('pg') //0805
...
const host = config.get('postgis.host')
const port = config.get('postgis.port')
...
for (const src of srcs) {
  ...
  const ogr2ogr = spawn(ogr2ogrPath, [
    ...
    '-lco', 'RS=YES',
    '/vsistdout/',
    `PG:"dbname='${pgDbname}' host='${pgHost}' port='${pgPort}' user='${pgUser}' password='${pgPassword}'"`,
    `-sql "${src.sql}"`    
  ])
